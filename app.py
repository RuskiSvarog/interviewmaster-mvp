...
PROMPT_FB = """
You are an interview coach for first-time job seekers.

Respond in JSON ONLY like:
{{"score": 1-5, "tips": ["tip1","tip2"]}}
...
# ---------- SESSION ----------
if "step" not in st.session_state: st.session_state.update(step=0, role=None, qa=[])
...
if st.session_state.role is None:
    ...
    if st.session_state.role: st.session_state.step = 1; st.experimental_rerun()
...
# ---------- Q&A LOOP ----------
    ...
    if st.button("Submit", key=f"btn{n}") and ans.strip():
        fb_raw = openai.ChatCompletion.create(
            model=MODEL,
            messages=[{"role":"system","content":PROMPT_FB.format(q=q, a=ans)}]
...
# ---------- SUMMARY ----------
    ...
    for i,(q,a,fb) in enumerate(st.session_state.qa,1):
        ...
        st.markdown("---")
    if st.button("Start over"): st.session_state.clear()
